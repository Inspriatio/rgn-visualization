...
def analysisGrade(phi,psi):
    if (phi == None or psi == None):
        grade = None
    elif((phi >= -165 and phi <= -55 and psi >= 100 and psi <= 180) or (phi >= -118 and phi <= -29 and psi >= -70 and psi <= 19) or (phi >= 49 and phi <= 72 and psi >= 29 and psi <= 50)):
        grade = "favoured"
    elif((phi >= -167 and phi <= -48 and psi >= -180 and psi <= -138 ) or (phi >= -180 and phi <= -29 and psi >= 68 and psi <= 180) or (phi >= -169 and phi <= -20 and psi >= -100 and psi <= 48) or (phi >= 32 and phi <= 90 and psi >= -10 and psi <= 88) or (phi >= 50 and phi <= 76 and psi >= -170 and psi <= -138)):
        grade = "allowed"
    elif ((phi >= -180 and phi <= 29 and psi >= 40 and psi <= 180) or (
            phi >= -180 and phi <= -30 and psi >= -180 and psi <= -121) or (
                  phi >= -180 and phi <= 29 and psi >= -109 and psi <= 40) or (
                  phi >= 30 and phi <= 110 and psi >= -30 and psi <= 118) or (
                  phi >= 30 and phi <= 105 and psi >= -180 and psi <= -119)):
        grade = "generously"
...
for a in range(len(FName)):
    ...
    for i in range(len(preAngel[0])):
        grade1 = analysisGrade(preAngel[0][i],preAngel[1][i])
        grade2 = analysisGrade(actAngel[0][i],actAngel[1][i])
        AllAngel_temp_pre.append([preAngel[0][i],preAngel[1][i],grade1])
        AllAngel_temp_act.append([actAngel[0][i],actAngel[1][i],grade2])
    AllAngel.append(AllAngel_temp_pre)
    AllAngel.append(AllAngel_temp_act)
